...
function AddProduct() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const responseManufacturers = await api.get(API_MANUFACTURER);
        setManufacturer(responseManufacturers.data);
      } catch (error) {
        console.log(error);
      }
    };
  ...
  const onSubmitForm = (formValue) => {
    ...
    if (formValue.image !== null) {
      formValue = {
        ...formValue,
        image: formValue.image[0].response.filename,
      };
      console.log(formValue);
    }
    if (!productId.id) {
        ...
        <Row>
          ...
          <Col md={11}>
            <Col md={18}>
              ...
              <Form.Item label="Manufacturer" name="manufacturerId" rules={[{ required: true }]}>
